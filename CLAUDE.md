...
- **Server config is env-driven** (`antlegion-bus/src/config.ts`, the `redis.conf` analog): `PORT` (28090), `HOST` (127.0.0.1), `ANTLEGION_DATA_DIR` (`.data-v2`), `ANTLEGION_FSYNC` (`always|everysec|no`, default `everysec`), `ANTLEGION_BUS_SECRET`, `ANTLEGION_MAX_DEPTH` (64), `ANTLEGION_CLAIM_TIMEOUT` (Δ, seconds). Set a **stable** `ANTLEGION_BUS_SECRET` — unset, the bus mints a fresh HMAC key each boot and `sig`s written before a restart can no longer be verified.
- **Δ is pinned to the log, not to the process (§8.4 M12).** The bus records it in `<dataDir>/log-meta.json` when the log is created, adopts that value on every later start, and **refuses to boot** when `ANTLEGION_CLAIM_TIMEOUT` disagrees with it. `ANTLEGION_CLAIM_TIMEOUT` unset means "no preference" (resolves to `undefined`, not 600), so an existing log keeps its own Δ. This is not a convenience check: every §8.4 result is a function of *(prefix, Δ)*, so serving one journal under two Δ can fold a `resolved` back to `open` — a terminal state undone with nothing appended. See `research/protocol-v3-audit-2026-08.md` §1.
...
- `research/` — first-party measurements the READMEs cite, plus `protocol-v3-feasibility-2026-08.md` (twelve isolated processes against one log), `protocol-v3-audit-2026-08.md` (adversarial audit of the spec's own claims), and `implementation-mvp-assessment-2026-08.md` (§10.2's MUSTs, one by one, against the code). `deploy/mvp/`, `toys/` — containerized multi-agent runs.
