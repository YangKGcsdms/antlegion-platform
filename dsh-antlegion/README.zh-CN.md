...
然后，在 `exclusive` 模式下，**运行时把活下来的事实认领掉——在唤醒任何东西之前**。
所有权是协议操作，不是决策：§3.1 规定赢家是最低 seq 的存活认领，这里没有一个字节需要
模型判断。代码认领意味着胜负在毫秒级由 seq 定下，输的那个 DCU **一次模型调用都不花**，
而真正跑起来的那一轮，一上来就被告知"这条已经归你了"。轮次结束由运行时 resolve，
不把收尾押在模型愿不愿意调工具上——如果这一轮没有在那条事实底下追加任何东西，运行时会
记一条 `dcu.no_output` 挂上去，而不是让它静默消失。长轮次靠每 Δ/3 重新认领来续住所有权；
进程死了就停止续期，认领自然过期，同伴接手。

`observe` 模式则一条都不认领：每个关心它的 DCU 都被同一条事实唤醒，各自把产出挂在它
底下——同一条事实的 N 个独立视角。这也是完全正当的姿态，而且**不再需要在提示词里跟简报
吵架**才能得到。
...
| `mode` | `exclusive` | 所有权怎么拿——**两种模式都在代码里拿，都不经过模型**。`exclusive`：巡检在唤醒会话之前先认领，轮次结束后由运行时收尾。`observe`：不认领，每个关心它的 DCU 都会被同一条事实唤醒 |
| `retryOnNoOutput` | `1` | 一轮没有在被唤醒的事实底下追加任何事实时，额外重述几次；`0` 表示直接记账 |
| `claimTimeoutSec` | `0` | 这个 DCU 折叠时用的认领过期 Δ；`0` 走 §8 默认值（600 秒）。它也决定轮次进行中的认领续期节奏（Δ/3） |
...
- 两种模式下总线工具都仍然挂着，所以模型**仍然能够**自己去调 `antlegion_claim` /
  `antlegion_resolve`。简报叫它别调、生命周期也由运行时掌管，但没有任何东西把工具摘掉。
