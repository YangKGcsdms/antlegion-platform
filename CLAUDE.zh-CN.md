...
- **`fold.ts` —— 读者折叠（语义）。** `current`/`history`（§8.1 的 subject 寄存器 —— 「X 现在是什么」）、`supersededBy`/`isSuperseded`、`causationChain`/`descendants`（§8.2 踪迹，双向；祖先未解析时给出显式缺口标记）、`lifecycle`（claimed/resolved/dead/open）、`claimWinner`/`didIWin`、`trust`，以及 §8.5 的可选约定：`colony`/`SYS_REGISTRY`（板上有谁）、`orphanReport`（没人关心的事实类型）、`contextGaps`（`context.requested`/`context.provided`）。对事实流的纯函数。
...
- **基于时间的折叠以 `recv`（总线盖，可信）为准，绝不用 `ts`（作者自报，仅供参考）。** 认领超时**以 recv 锚定、确定性**：一个 claim 在后续某事实的 `recv` 越过 `claim.recv + Δ` 时过期；只有末尾无后继的 claim 才回退到墙钟 `now`。这使崩溃恢复重派能转移 owner、又不撤销真实 resolve（`PROTOCOL.md` §8.4、§9.3）。
